...
// Збереження вмісту кошика до файлу
...
// Роут для оновлення кількості товару у кошику
app.post('/cart', (req, res) => {
    const { id, quantity } = req.body;

    if (!id || quantity === undefined || isNaN(quantity) || quantity < 0) {
    ...
    if (quantity === 0) {
        cart = cart.filter(item => item.id !== id); // Видалення з корзини
